...
const Option: React.FC<OptionProps> = ({
  ...
  return (
    <TouchableOpacity
      ...
      className={`flex-row h-16 w-full items-center rounded-lg mb-4 border border-[#76767650] ${
        selectedOption === index ? "bg-[#3c3d3d]" : "bg-[#767676]"
      }`}
      onPress={() => setSelectedOption(index)}
    >
      <View
        className={`rounded-xl h-10 w-10 mr-2 justify-center items-center ml-5 border border-[#76767650] ${
          selectedOption === index ? "bg-[#767676]" : "bg-[#3c3d3d]"
        }`}
      >
        <Text
      ...
      <View className="flex-1 mr-14 justify-center items-center">
        <Text
          ...
          className={`text-center text-lg ${
            selectedOption === index ? "text-white" : "text-white"
          }`}
        >
          {option}
        </Text>
...
const Quiz = () => {
  ...
  const { getUserDetails, userProfile, setUserProfile } = useUser();
          ...
          <CustomAlert
            ...
            onClose={async () => {
              ...
              try {
                ...
                await api.put(`/api/usuario/${user?.email}`, {
