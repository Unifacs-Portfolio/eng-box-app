...
const ChangeUsernameScreen = () => {
	...
	const onSubmit = async (data: { username: string }) => {
		try {
			...
			// Verifica se já existe o nome de usuário (ignorando o próprio usuário)
			const usernameExists = usuarios.some(
				(u: any) =>
					u.nome?.toLowerCase() === data.username.toLowerCase() &&
			...
			setUserProfile({ ...userProfile, nome: data.username });
			setIsChanged(false);
	...
	return (
		<SafeAreaView className="flex-1 mt-4">
			...
			>
				Para alterar o nome de usuário, clique aqui
			</Text>
					...
					<Controller
						...
						rules={{
							...
							validate: {
								...
								noSpecialCharsEnd: (value) =>
									!/[._]$/.test(value) ||
									'Não deve terminar com caracteres especiais',
								onlyAllowedSpecialChars: (value) =>
									/^[a-zA-Z0-9._]*$/.test(value) ||
									'Use apenas . e _ como caracteres especiais',
								notAllUppercase: (value) =>
									value !== value.toUpperCase() ||
									'O nome de usuário não pode ser totalmente em maiúsculo',
